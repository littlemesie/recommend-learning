...
def title_corpus(title):
    ...
    for t in title:
        text = t['title']
        for te in text.lower().replace('\n', '').split(' '):
            if te and te not in stop_list and not te.isdigit():
                tt.append(te)
        # tt.append(str(te) for te in text.replace('\n', '').split(' ') if te)
