...
def split_data(data_file, test_size):
    # 加载数据集
...
def calc_similarity(n_users, n_items, train_data, test_data):
    ...
    return train_data_matrix, test_data_matrix, item_popular
...
def evaluate(prediction, item_popular, name, train_data_matrix, test_data_matrix):
    hit = 0
    rec_count = 0
    test_count = 0
    popular_sum = 0
...
def recommend(u_index, prediction, train_data_matrix, test_data_matrix):
    items = np.where(train_data_matrix[u_index, :] == 0)[0]
    pre_items = sorted(
        dict(zip(items, prediction[u_index, items])).items(),
        key=itemgetter(1),
...
if __name__ == "__main__":
    ...
    df, n_users, n_items, train_data, test_data = split_data(data_file, test_size=0.25)
    print(n_users)
    # 计算相似度
    # train_data_matrix, test_data_matrix, item_popular = calc_similarity(n_users, n_items, train_data, test_data)
