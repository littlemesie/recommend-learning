...
import io
"""
参考https://github.com/KKeishiro/Yahoo_recommendation
"""
class LinUCB:
    def __init__(self):
        self.alpha = 0.25
    ...
    def set_articles(self, art):
        for key in art:
            ...
            self.theta[key] = np.transpose([art[key]])
...
if __name__ == '__main__':
    """"""
    log_file = '../../data/brandit/webscope-logs.txt'
    articles_file = '../../data/brandit/webscope-articles.txt'
    model = LinUCB()
    ...
    for art in articles_np:
        articles_info[int(art[0])] = [float(x) for x in art[1:]]
        articles.add(int(art[0]))
    user_features = []
    with io.open(log_file, 'rb', buffering=1024*1024*512) as inf:
        for line in inf:
            arr = line.strip().split()
            user_features.append([float(x) for x in arr[1:7]])
    model.set_articles(articles_info)
    # print(model.theta)
    for i in range(10):
        model.recommend(user_features[0], list(articles))
        model.recommend(user_features[1], list(articles))
    # print(model.a_max)
    # model.recommend(user_features[1], list(articles))
    # print(model.a_max)
    # model.recommend(user_features[1], list(articles))
