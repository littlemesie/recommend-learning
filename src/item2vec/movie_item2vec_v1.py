...
class Item2Vec(object):
    ...
    def predict(self, item_index_data, quey_id,  top_N=10):
        """"""
        return self.similar_items(self.embed, item_index_data, quey_id, N=top_N)


if __name__ == '__main__':
    from item2vec.utils.movie_process import MovieProcessor_V1
    config = {
        "embedding_size": 256,
        "num_negatives": 30,
        "learning_rate": 0.5,
    ...
    processor = MovieProcessor_V1()
    config['processor'] = processor
    model = Item2Vec(**config)
    user_item_index_data = []
    user_item_map = {}
    index = 0
    for user_id, index_data in processor.user_item_index_data.items():
        user_item_index_data.append(index_data)
        user_item_map[index] = user_id
        index += 1
    model.fit(user_item_index_data)
    result = model.predict(user_item_index_data, 10)
